...
def get_criterion(num_classes: int, use_gpu: bool, args):
    if args.criterion == 'htri':
        ...
        from torchreid.losses.cross_entropy_loss import ABD_CrossEntropyLoss
        criterion = ABD_CrossEntropyLoss(num_classes, use_gpu=use_gpu, label_smooth=args.label_smooth)
    else:
...
def main():
    ...
    model = models.build_model(name=args.arch, num_classes=dm.num_train_pids, loss={'xent'}, use_gpu=use_gpu,
                               args=vars(args))
    print(model)
        ...
        for name in args.target_names:
            ...
            if args.visualize_ranks:
                visualize_ranked_results(
                    distmat, (dm.test_dataset[name]['query'], dm.test_dataset[name]['gallery']),
        ...
        if (epoch + 1) > args.start_eval and args.eval_freq > 0 and (epoch + 1) % args.eval_freq == 0 or (
            ...
            if max_r1 < rank1:
                ...
                save_checkpoint({
                    ...
                    'rank1': rank1,
                    'epoch': epoch,
                }, osp.join(args.save_dir, 'checkpoint_best.pth.tar'), True)
